...
namespace Noise
{
	...
	public static class Noise
	{
		...
		public static float GetOctaveNoise(double pX, double pY, double pZ, int pOctaves)
		{
			...
			{
				currentHalf = (float)Math.Pow(0.35f, i);
		...
		public static float GetTurbulentNoise(double pX, double pY, double pZ, int pOctaves, double radius, double elevation)
		{
			double	noise = (float)GetOctaveNoise(pX, pY, pZ, pOctaves);
			double	dist = Math.Pow(pX, 2) + Math.Pow(pY, 2) + Math.Pow(pZ, 2);

			return (float)(noise * (1 - (dist - radius) / (elevation)));
		}
	}
}